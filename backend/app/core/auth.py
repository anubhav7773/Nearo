    ...
    if _jwks_client is None:
        ...
        if jwks_url:
            try:
                ...
                logger.warning(f"Failed to initialize PyJWKClient with URL {jwks_url}: {e}")
                _jwks_client = None
    return _jwks_client
...
def decode_clerk_jwt(token: str) -> Dict[str, Any]:
    ...
    raise jwt.InvalidTokenError("Could not validate Clerk JWT token with JWKS or secret key")
...
def extract_clerk_user_claims(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not phone_number and isinstance(payload.get("phone_numbers"), list) and payload["phone_numbers"]:
